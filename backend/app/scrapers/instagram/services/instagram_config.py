...
class Config:
    ...
    MAX_WORKERS = int(
        os.getenv("INSTAGRAM_MAX_WORKERS", "10")
    )  # Concurrent threads - optimized for performance
    REQUESTS_PER_SECOND = int(
        os.getenv("INSTAGRAM_REQUESTS_PER_SECOND", "55")
    )  # Stay under 60 limit
    RATE_LIMIT_DELAY = 1.0 / REQUESTS_PER_SECOND  # Calculate delay between requests

    # Batch Processing
    BATCH_SIZE = int(
        os.getenv("INSTAGRAM_BATCH_SIZE", "50")
    )  # Process 50 creators per batch - reduced for stability
    CONCURRENT_CREATORS = int(
        os.getenv("INSTAGRAM_CONCURRENT_CREATORS", "20")
    )  # Process 20 creators simultaneously (v3.12.0 - 2x increase for 10-20x speedup)

    # Connection Pooling
    CONNECTION_POOL_SIZE = int(
        os.getenv("INSTAGRAM_CONNECTION_POOL_SIZE", "20")
    )  # 2x the thread count for efficiency
    ...
    RETRY_MAX_ATTEMPTS = int(os.getenv("RETRY_MAX_ATTEMPTS", "3"))  # Keep at 3 (faster failure)
    ...
    ENABLE_WEBHOOK_NOTIFICATIONS = (
        os.getenv("ENABLE_WEBHOOK_NOTIFICATIONS", "false").lower() == "true"
    )
