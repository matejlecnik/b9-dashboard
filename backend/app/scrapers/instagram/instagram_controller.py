...
class ContinuousInstagramScraper:
    ...
    async def update_heartbeat(self):
        ...
        try:
            self.supabase.table("system_control").update(
                {"last_heartbeat": datetime.now(timezone.utc).isoformat(), "pid": os.getpid()}
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                if self.last_check is not None and self.last_check != enabled:
                    status_msg = (
                        "✅ Instagram scraper ENABLED"
                        if enabled
                        else "⏹️ Instagram scraper DISABLED"
                    )
                ...
                logger.warning(
                    "No Instagram scraper control record found, creating default (disabled)"
                )
    ...
    async def run_scraping_cycle(self):
        ...
        try:
            self._log_to_system(
                "info",
                f"Starting Instagram scraping cycle #{self.cycle_count}",
                {"cycle": self.cycle_count, "version": SCRAPER_VERSION},
            )
            ...
            creators_processed = (
                self.scraper.creators_processed
                if self.scraper and hasattr(self.scraper, "creators_processed")
                else 0
            )
            api_calls = (
                self.scraper.api_calls_made
                if self.scraper and hasattr(self.scraper, "api_calls_made")
                else 0
            )
            ...
            self._log_to_system(
                "success",
                f"Completed Instagram cycle #{self.cycle_count} in {duration_str}",
                {
            ...
            logger.info(
                f"✅ Cycle #{self.cycle_count} complete - {creators_processed} creators processed"
            )
    ...
    async def run_continuous(self):
        ...
        logger.info(
            f"🚀 Starting Instagram scraper v{SCRAPER_VERSION} with 30-second check interval"
        )
        ...
        # Log startup
        self._log_to_system(
            "info", f"Instagram scraper v{SCRAPER_VERSION} started", {"version": SCRAPER_VERSION}
        )
            ...
            try:
                ...
                if enabled:
                    ...
                    if self.next_cycle_at and now < self.next_cycle_at:
                        ...
                        # Log every minute
                        if (
                            not hasattr(self, "_last_wait_minute")
                            or self._last_wait_minute != wait_minutes
                        ):
                            ...
                            self._log_to_system(
                                "info",
                                f"⏳ Waiting {wait_minutes} minutes until next cycle",
                                {
                        ...
                        self._log_to_system(
                            "info",
                            f"Starting {CYCLE_WAIT_HOURS}-hour wait period",
                            {
