...
class PublicRedditAPI:
    ...
    def __init__(
        self, max_retries: int = 5, base_delay: float = 1.0, thread_id: Optional[int] = None
    ):
        """
    ...
    def request_with_retry(self, url: str, proxy_config: Optional[Dict] = None) -> Optional[Dict]:
        ...
        if not proxy_config:
            logger.error(
                "❌ PROXY REQUIRED: No proxy configuration provided - direct API access is FORBIDDEN"
            )
            return None
        ...
        if not proxies:
            logger.error(
                "❌ PROXY REQUIRED: Failed to configure proxy - direct API access is FORBIDDEN"
            )
            return None
        ...
        while retries < self.max_retries:
            try:
                ...
                if self.session is None or self._request_count >= 100:
                    ...
                    self.session.mount(
                        "http://",
                        requests.adapters.HTTPAdapter(pool_connections=5, pool_maxsize=10),
                    )
                    self.session.mount(
                        "https://",
                        requests.adapters.HTTPAdapter(pool_connections=5, pool_maxsize=10),
                ...
                if response.status_code == 403:
                    if logger_helper:
                        logger_helper.warning(
                            f"Forbidden: {url}",
                            context={"url": url, "status_code": 403, "thread_id": self.thread_id},
                            action="forbidden",
                        )
                ...
                # Success
                logger.debug(
                    f"Request successful: {url} - {response.status_code} in {response_time_ms}ms"
                )
                ...
                if retries < self.max_retries:
                    logger.warning(
                        f"Request failed (attempt {retries}/{self.max_retries}): {url} - {str(e)[:100]}"
                    )
...
class ThreadSafeAPIPool:
    ...
    def initialize(self, thread_count: Optional[int] = None):
        ...
        for thread_id in range(thread_count):
            ...
            self.apis[thread_id] = PublicRedditAPI(
                max_retries=5, base_delay=1.0, thread_id=thread_id
            )
    ...
    def get_api(self, thread_id: int) -> Optional[PublicRedditAPI]:
        ...
        if thread_id not in self.apis:
            logger.warning(
                f"No API instance for thread {thread_id}. Available threads: {list(self.apis.keys())}"
            )
            return None
    ...
    def get_api_with_proxy(
        self, thread_id: int
    ) -> Tuple[Optional[PublicRedditAPI], Optional[Dict]]:
        """
