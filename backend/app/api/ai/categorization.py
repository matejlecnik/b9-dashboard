...
async def tag_subreddits(request: Request, body: TagSubredditsRequest):
    ...
    try:
        logger.info(
            f"🎯 Starting AI categorization: limit={body.limit}, batch_size={body.batch_size}"
        )
        ...
        result = await service.tag_all_uncategorized(
            batch_size=body.batch_size or 50, limit=body.limit, subreddit_ids=body.subreddit_ids
        )
        ...
        logger.info(
            f"✅ Categorization complete: {stats.get('successful', 0)} tagged, ${stats.get('total_cost', 0):.4f} cost"
        )

        return TagSubredditsResponse(success=success, message=message, stats=stats)
