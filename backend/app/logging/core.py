...
class UnifiedLogger:
    ...
    def _setup_handlers(self, supabase_client):
        ...
        if config.console_logging_enabled:
            console_handler = setup_console_handler(use_json=(config.console_format == "json"))
    ...
    def info(
        ...
        **kwargs,
        ...
        extra = self._add_context(
            {"context": context, "action": action, "duration_ms": duration_ms}
        )
...
def get_logger(
    name: str, supabase_client=None, source: str = "api", script_name: Optional[str] = None
) -> UnifiedLogger:
    """
    ...
    if cache_key not in _logger_cache:
        _logger_cache[cache_key] = UnifiedLogger(
            name=name, supabase_client=supabase_client, source=source, script_name=script_name
        )
