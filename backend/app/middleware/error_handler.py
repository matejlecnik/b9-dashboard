...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def handle_http_exception(self, request: Request, exc: HTTPException) -> JSONResponse:
        ...
        return JSONResponse(
            status_code=exc.status_code, content=error_data, headers=getattr(exc, "headers", None)
        )

    async def handle_starlette_exception(
    ...
    async def log_error_to_database(
        self, error_id: str, request: Request, exception: Exception, traceback_str: str
    ):
        ...
        try:
            if SUPABASE_AVAILABLE:
                ...
                logger.info(
                    f"Structured error log (Supabase unavailable): {json.dumps(error_log, indent=2)}"
                )
        ...
        return JSONResponse(
            ...
            content={
                ...
                "available_endpoints": [
                    ...
                    "/api/scraper/stop",
                    "/api/scraper/status",
                    "/api/scraper/status-detailed",
                    "/api/scraper/cycle-status",
                    # Instagram scraper
                    "/api/instagram/scraper/start",
                    "/api/instagram/scraper/stop",
                    "/api/instagram/scraper/status",
                    "/api/instagram/scraper/cycle-status",
                    # Categorization
                    "/api/categorization/start",
...
def log_and_raise_error(
    ...
    raise HTTPException(
        status_code=status_code,
        detail={"message": message, "error_type": error_type, "details": details},
    )
