...
class BaseScraper(ABC):
    ...
    async def save_to_database(self, data_type: str, data: Any):
        ...
        try:
            ...
            if data_type == 'subreddit':
                ...
                subreddit = data.get('discovered_subreddit', 'unknown')
                logger.info(f"💾 [Thread {self.thread_id}] Saving discovery of r/{subreddit}")
                await self.batch_writer.add_discovered_subreddit(data)
            else:
    ...
    def should_continue(self, control_checker: Optional[Callable] = None) -> bool:
        ...
        Args:
            control_checker: Optional external control check function
        ...
        # Check external control if provided
        if control_checker:
            try:
                import asyncio
                # Check if it's a coroutine function
                if asyncio.iscoroutinefunction(control_checker):
                    # Run the coroutine in the current event loop if available
                    try:
                        ...
                        return asyncio.run(control_checker())
                else:
