...
import { addCorsHeaders as withCors, type CorsOptions, corsResponse } from '@/lib/cors'
...
export function withApiSecurity<T extends unknown[]>(
  ...
  return async (request: NextRequest, ...args: T): Promise<NextResponse | Response> => {
    try {
      ...
      // Check if this is a public route (simple heuristic)
      const pathname = request.nextUrl.pathname
      const isPublic = pathname.startsWith('/public/') || pathname.includes('/health')
...
export const publicApi = <T extends unknown[]>(
