...
async def log_creator_addition(username: str, action: str, success: bool,
    ...
    try:
        ...
        log_entry = {
            ...
            'message': f"Manual creator add: {action} for @{username}" + (f" - {error}" if error else ""),
            'context': context
        }
...
async def add_instagram_creator(request: CreatorAddRequest, background_tasks: BackgroundTasks):
    ...
    if not username:
        await log_creator_addition(username, 'validation_failed', False,
    ...
    try:
        ...
        await log_creator_addition(username, 'scraper_init', True,
                                  {'source': 'manual_add'})
        ...
        if not profile_data:
            await log_creator_addition(username, 'profile_fetch_failed', False,
        ...
        if not ig_user_id:
            await log_creator_addition(username, 'ig_user_id_missing', False,
                                      error="Failed to extract Instagram user ID from profile")
            return CreatorAddResponse(
                success=False,
                error="Failed to get Instagram user ID from profile"
            )

        await log_creator_addition(username, 'profile_fetched', True,
        ...
        if not existing_creator or not hasattr(existing_creator, 'data') or not existing_creator.data:
            ...
            if not insert_result or not hasattr(insert_result, 'data') or not insert_result.data:
                ...
                return CreatorAddResponse(
                    success=False,
                    error="Failed to create creator record in database"
                )

            await log_creator_addition(username, 'creator_inserted', True,
                                      {'ig_user_id': ig_user_id})
            ...
            _get_db().table('instagram_creators')\
                .update(update_data)\
                .eq('ig_user_id', ig_user_id)\
                .execute()

            await log_creator_addition(username, 'creator_updated', True,
                                      {'ig_user_id': ig_user_id})
        ...
        await log_creator_addition(username, 'full_processing_started', True,
        ...
        if not processing_success:
            await log_creator_addition(username, 'processing_failed', False,
        ...
        await log_creator_addition(username, 'processing_completed', True,
                                  {'api_calls_used': api_calls_used})

        # 7. Fetch complete creator data from database
        final_creator = _get_db().table('instagram_creators')\
            .select('*')\
            .eq('ig_user_id', ig_user_id)\
            .single()\
            .execute()
        ...
        await log_creator_addition(username, 'addition_completed', True,
                                  details={
                                      'ig_user_id': ig_user_id,
        ...
        logger.info(f"✅ Successfully added creator @{username}: "
                   f"{api_calls_used} API calls, {processing_time}s processing time")

        # 9. Return success response
        return CreatorAddResponse(
            success=True,
            creator=final_creator.data,
            stats=stats
        )
        ...
        # Log error
        await log_creator_addition(username, 'addition_error', False,
                                  error=error_msg,
